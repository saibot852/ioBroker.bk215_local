...
function getMac(callback) {
	...
	require('node:child_process').exec(command, (err, stdout) => {
		if (err) {
			callback(err);
		} else {
			let macAddress;
...
function uuid(givenMac, callback) {
	...
	if (mac === '') {
		getMac((err, mac) => uuid(mac || null, callback));
		return;
	}
 ...
 * @returns {{ license: string, link?: string, type?: string }}
 */
...
function getInstalledInfo(hostRunningVersion) {
	...
	if (
		fs.existsSync(`${__dirname}/../../../node_modules/${module.exports.appName.toLowerCase()}.js-controller`) ||
		...
		for (i = 0; i < dirs.length; i++) {
			try {
				...
				if (
					regExp.test(dirs[i]) &&
					dirs[i].substring(module.exports.appName.length + 1) !== 'js-controller' &&
...
function getNpmVersion(adapter, callback) {
	...
	exec(cliCommand, { timeout: 2000 }, (error, stdout) => {
		let version;
		if (error) {
			// command failed
...
function sendDiagInfo(obj, callback) {
	axios
		.post(`http://download.${module.exports.appName}.net/diag.php`, `data=${JSON.stringify(obj)}`, {
...
function getAdapterDir(adapter, isNpm) {
	...
	let dir = parts.join('/');
	if (adapter.substring(0, module.exports.appName.length + 1) === `${module.exports.appName}.`) {
		adapter = adapter.substring(module.exports.appName.length + 1);
	}

	if (
		fs.existsSync(`${dir}/node_modules/${module.exports.appName}.js-controller`) &&
...
function getSystemNpmVersion(callback) {
	...
	exec('npm -v', { encoding: 'utf8', env: newEnv }, (error, stdout) => {
		if (stdout) {
