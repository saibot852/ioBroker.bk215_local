...
const { addLabel, createLabel, deleteLabel, updateLabel, getLabels, getAllComments, getGithub } = require('./common');
...
async function handleOthers(pIssues) {
	for (const issue of pIssues) {
		if (!issue.labels.find(label => label.name === 'STABLE - brand new')) {
			...
			let comment = comments.findLast(c => /reminder (\d+\.\d+\.\d+|none)/g.exec(c.body));
