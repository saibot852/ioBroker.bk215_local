...
const { addComment, getGithub, getUrl, createIssue, deleteComment, getAllComments } = require('./common');

async function getLatestRepo() {
	return await getUrl('https://download.iobroker.net/sources-dist-latest.json');
}

async function getStableRepo() {
	return await getUrl('https://download.iobroker.net/sources-dist.json');
}
...
async function updateSummaryIssue(adapters) {
	...
	issues = await getGithub(
		`https://api.github.com/repos/ioBroker/ioBroker.repositories/issues?state=open&page=1&per_page=100`,
	);
	issues = issues.filter(i => i.state === 'open' && i.title.includes(title));
	if (!issues.length) {
