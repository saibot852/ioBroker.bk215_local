...
async function doIt() {
	...
	let exists = gitComments.find(comment =>
		comment.body.includes('## ioBroker repository information about STABLE-BRAND-NEW tagging'),
	);
	...
	if (!exists && labelIsSet) {
		...
		body += `Your PR will be kept in evidence and will be merged approximately one or two weeks after creation of the release without any further `;
