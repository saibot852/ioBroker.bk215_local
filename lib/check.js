...
async function detectChanges(commit) {
	...
	const patch = commit.patch
		...
		.map(t => t.trim())
		.filter(t => t);
	if (commit.changes > 75) {
		/* Mcm1957: limit changed from 25 to 75. Resorting might create a bigger number of changed lines */
	...
	return adapters.map(a =>
		repo[a].meta.replace(/\/master\/io-package.json$/, '').replace(/\/main\/io-package.json$/, ''),
	);
}
...
async function detectAffectedAdapter(prID) {
	...
	for (let i = 0; i < commits.length; i++) {
		const commit = await getGithub(
			`https://api.github.com/repos/ioBroker/ioBroker.repositories/commits/${commits[i].sha}`,
		);
