...
async function doIt() {
	...
	let exists = gitComments.find(comment =>
		comment.body.includes('## ioBroker repository information about STABLE-0-Day-PR tagging'),
	);
	...
	if (!exists && labelIsSet) {
		...
		body += `**Your PR might be closed for now.** Please create a new PR after the new version has been available at LATEST repository `;
		body += `for at least one or two weeks, has a considerable number of installations and no serious new issues in this case. If the PR is kept open it will `;
		body += `be processed after the new release has been available at latest one or two weeks and has some users.\n\n`;
