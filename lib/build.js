...
function getStats(cb) {
	if (cb) {
		...
		return axios('http://iobroker.live/statistics.json').then(response => response.data.adapters);
	}
}
...
function getImages(list, destination, callback) {
	if (!list || !list.length) {
		...
		if (!fs.existsSync(`${destination + name.name}.svg`)) {
			...
			axios(`http://img.shields.io/npm/v/iobroker.${name.name}.svg?style=flat-square`, {
				...
				.then(response => {
					if (response.data) {
						...
						console.error(
							`Got empty file "http://img.shields.io/npm/v/iobroker.${name.name}.svg?style=flat-square`,
						);
				...
				.catch(error => {
					console.error(
						`Cannot get URL "http://img.shields.io/npm/v/iobroker.${name.name}.svg?style=flat-square: ${error}`,
					);
					setTimeout(getImages, 100, list, destination, callback);
				});
		} else {
