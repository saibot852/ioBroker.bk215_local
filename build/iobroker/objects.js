...
exports.ensureObjects = ensureObjects;
async function ensureNumber(adapter, id, name, role, unit, write = false, min, max, step, def) {
...
async function ensureBool(adapter, id, name, write = false) {
    await adapter.setObjectNotExistsAsync(id, {
        type: 'state',
        common: {
            name,
...
async function ensureText(adapter, id, name, write = false, role = 'text') {
    await adapter.setObjectNotExistsAsync(id, {
        type: 'state',
        common: {
            name,
...
async function ensureBatterySocStates(adapter) {
...
async function ensureBmsLimitStates(adapter) {
...
async function ensureConfigStates(adapter) {
    await ensureNumber(adapter, 'config.system_discharge_limit', 'System discharge limit (min SOC)', 'level', '%', true, 1, 20, 1);
    await ensureNumber(adapter, 'config.system_charge_limit', 'System charge limit (max SOC)', 'level', '%', true, 70, 100, 1);
    ...
    await ensureNumber(adapter, 'config.battery_charge_cutoff', 'Battery charge cutoff', 'level', '%', true, 80, 100, 1);
    await ensureNumber(adapter, 'config.system_charging_power', 'System charging power', 'level', 'W', true, 0, 3600, 1);
    await ensureNumber(adapter, 'config.idle_shutdown_time', 'Idle shutdown time', 'level', 'min', true, 15, 1440, 1);
    await ensureNumber(adapter, 'config.low_battery_shutdown_time', 'Low battery shutdown time', 'level', 'min', true, 5, 1440, 1);
}
async function ensureModeStates(adapter) {
...
async function ensureEnergyStates(adapter) {
...
async function ensurePvStates(adapter) {
    for (let i = 1; i <= 9; i++) {
        await ensureNumber(adapter, `status.pv${i}_input_power`, `PV${i} input power`, 'value.power', 'W');
    }
}
async function ensureTemperatureStates(adapter) {
...
async function ensureMpptStates(adapter) {
...
async function ensureHeaterStates(adapter) {
...
async function ensureDiagnosticStates(adapter) {
